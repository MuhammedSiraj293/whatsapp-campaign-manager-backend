...
const checkAndSendFollowUps = async () => {
  console.log("🔄 FollowUpScheduler: Running Smart Checks (v2.1 optimized)");
  const now = Date.now();

  try {
    // ------------------------------------------------------------------
    // STEP 0: PRE-CHECK ENABLED NUMBERS (OPTIMIZATION)
    // ------------------------------------------------------------------
    // Fetch only the phone numbers that have features ENABLED.
    // This prevents the scheduler from queried thousands of enquiries for disabled lines.
    const enabledFollowUpDocs = await PhoneNumber.find({
      isFollowUpEnabled: true,
    }).select("phoneNumberId");

    const enabledReviewDocs = await PhoneNumber.find({
      isReviewEnabled: true,
    }).select("phoneNumberId");

    const followUpRecipientIds = enabledFollowUpDocs.map(
      (d) => d.phoneNumberId,
    );
    const reviewRecipientIds = enabledReviewDocs.map((d) => d.phoneNumberId);

    if (followUpRecipientIds.length === 0 && reviewRecipientIds.length === 0) {
      // console.log("💤 No phone numbers have follow-up/review enabled. Skipping checks.");
      return;
    }
    ...
    // ------------------------------------------------------------------
    if (followUpRecipientIds.length > 0) {
      // Find active enquiries updated > 3 mins ago meant for ENABLED numbers
      ...
      const stuckEnquiries = await Enquiry.find({
        ...
        recipientId: { $in: followUpRecipientIds }, // <--- FILTER BY ENABLED IDS
    ...
    if (followUpRecipientIds.length > 0) {
      ...
      // AND meant for ENABLED numbers
      const timeoutEnquiries = await Enquiry.find({
        conversationState: { $ne: "END" },
        status: { $ne: "closed", $ne: "handover" },
        recipientId: { $in: followUpRecipientIds }, // <--- FILTER BY ENABLED IDS
        lastStuckFollowUpSentAt: { $lt: tenMinutesAgo, $ne: null },
    ...
    if (reviewRecipientIds.length > 0) {
      ...
      const reviewCandidates = await Enquiry.find({
        ...
        recipientId: { $in: reviewRecipientIds }, // <--- FILTER BY ENABLED IDS
        endedAt: { $lt: oneMinuteAgo },
      });
